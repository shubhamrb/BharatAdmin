...
public class EditLiveClassActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        liveModel = getIntent().getParcelableExtra("LIVE_CLASS");
        userSelectedDays = getIntent().getIntegerArrayListExtra("scheduleDay");
    ...
    private void initUI() {
        ...
        for (int i = 0; i < 7; i++) {
            if (userSelectedDays.contains(i)) {
                checkedDays[i] = true;
            }
        }
        etLiveClassName.setText(liveModel.getLiveClassName());
        etMeetingUrl.setText(liveModel.getMeetingUrl());
        etMeetingId.setText(liveModel.getMeetingId());
        etTeacherName.setText(liveModel.getTeacherName());
        ...
        spinScheduleTime.setText(liveModel.getScheduleTime());
        realTime = liveModel.getScheduleRealTime();
        etTeacherName.setText(liveModel.getTeacherName());

        spinScheduleDay.setOnClickListener(view -> onDayClicked());
        spinScheduleTime.setOnClickListener(view -> onTimeClicked());
        btnCreateClass.setOnClickListener(view -> editLiveClass());
    }
...
//            Toast.makeText(this, "" + hourOfDay + " : " + min, Toast.LENGTH_SHORT).show();
    ...
    private void editLiveClass() {
        ...
        if (auth.getCurrentUser() != null) {
            ...
            documentReference = db.collection("classes")
                    .document(classId).collection("liveClasses").document(liveModel.getId());


            batch.update(documentReference, "liveClassName", liveClassName);
            batch.update(documentReference, "meetingUrl", meetingUrl);
            batch.update(documentReference, "meetingId", meetingId);
            batch.update(documentReference, "teacherName", teacherName);
            batch.update(documentReference, "scheduleTime", scheduledTime);
            batch.update(documentReference, "scheduleRealTime", realTime);
            batch.update(documentReference, "scheduleDay", userSelectedDays);
    ...
    private void deleteLiveClass(LiveModel liveModel) {
        FirebaseFirestore db = FirebaseFirestore.getInstance();
        FirebaseAuth auth = FirebaseAuth.getInstance();
        liveModel.setDeleted(true);
        loadingView.setVisibility(View.VISIBLE);
        if (auth.getCurrentUser() != null) {
            db.collection("classes")
                    .document(classId).collection("liveClasses")
                    .document(liveModel.getId())
                    .update("deleted", true)
                    .addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void aVoid) {
                            Log.d(TAG, "LiveClass successfully Deleted!");
                            loadingView.setVisibility(View.GONE);
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case R.id.action_delete:
                ...
                materialAlertDialogBuilder.setCancelable(true);
                materialAlertDialogBuilder.setPositiveButton("Yes", (dialogInterface, i) -> deleteLiveClass(liveModel));
